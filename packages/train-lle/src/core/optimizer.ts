...
export class AdamW implements Optimizer {
  ...
  constructor(lr: number = 0.001, beta1: number = 0.9, beta2: number = 0.999, epsilon: number = 1e-8, weightDecay: number = 0.01) {
    this.lr = lr;
    this.beta1 = beta1;
    this.beta2 = beta2;
    this.epsilon = epsilon;
  ...
  step(params: Tensor[], grads: Tensor[]): void {
    ...
    for (let i = 0; i < params.length; i++) {
      // Weight decay
      params[i].data = params[i].mul(1 - this.lr * this.weightDecay).data;
